...
$entry = Join-Path $PSScriptRoot "labeling_ui.py"
...
try {
    ...
    Write-Host ""
    Write-Host "CABF labeling UI failed: PySide6 is missing in the selected Python environment." -ForegroundColor Red
...
try {
    ...
    Write-Host ""
    Write-Host "CABF labeling UI runtime error:" -ForegroundColor Red
    Write-Host $_
    Write-Host ""
    Read-Host "Press Enter to exit"
    exit 1
