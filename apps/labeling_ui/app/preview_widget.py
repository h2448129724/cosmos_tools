...
class ZoomableLabel(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._fixed_rect_size = None

        # Keep the established usable canvas floor; the parent splitter can
        # still allocate additional space responsively around it.
    ...
    def set_fixed_rect_size(self, size: tuple[int, int] | None) -> None:
        """Use an exact size for newly placed rectangles; unset for freehand drawing."""
        self._fixed_rect_size = size
        self.update()
    ...
    def _find_rect_corner_at(self, sx: float, sy: float) -> tuple[int, str]:
        if self._fixed_rect_size is not None or self._rect_selected_index < 0:
            return -1, ""
        handle_radius = 8
        corners = ("tl", "tr", "br", "bl")
        for index in (self._rect_selected_index,):
            x1, y1, x2, y2 = self._rects_image[index]
            handle_points = {
    ...
    def _fixed_rect_at(self, coordinate: tuple[int, int]) -> tuple[int, int, int, int]:
        width, height = self._fixed_rect_size or (1, 1)
        if self._pixmap is not None and not self._pixmap.isNull():
            width = min(width, self._pixmap.width())
            height = min(height, self._pixmap.height())
        x1, y1 = coordinate
        return self._clamp_rect_to_image((x1, y1, x1 + width, y1 + height))
    ...
    def paintEvent(self, event):
        ...
        if self._rect_select_mode and self._rect_drawing and self._rect_start_screen and self._rect_current_screen:
            if self._fixed_rect_size is not None:
                current = self._screen_to_image(*self._rect_current_screen)
                if current is None:
                    return
                x1, y1, x2, y2 = self._fixed_rect_at(current)
                rx, ry = self._image_to_screen(x1, y1)
                rw, rh = (x2 - x1) * self._zoom, (y2 - y1) * self._zoom
            else:
    ...
    def mouseReleaseEvent(self, event: QMouseEvent):
        ...
        if self._rect_drawing and self._rect_start_image:
            ...
            if end_coord:
                if self._fixed_rect_size is not None:
                    x1, y1, x2, y2 = self._fixed_rect_at(end_coord)
                else:
...
class PreviewPanel(QWidget):
    ...
    def _setup_ui(self):
        ...
        layout.addWidget(self._build_zoom_toolbar())
    ...
    def _build_zoom_toolbar(self):
        toolbar = ActionBar()
        toolbar.setAccessibleName("预览缩放工具栏")
        self.btn_fit = QPushButton("适应窗口")
        self.btn_fit.setAccessibleName("适应窗口")
        self.btn_fit.setToolTip("将当前图像适配到预览区域")
        self.btn_fit.clicked.connect(self._on_fit)
        self.btn_zoom_in = QPushButton("+")
        self.btn_zoom_in.setAccessibleName("放大预览")
        self.btn_zoom_in.setToolTip("放大预览")
        self.btn_zoom_in.clicked.connect(self._on_zoom_in)
        self.btn_zoom_out = QPushButton("-")
        self.btn_zoom_out.setAccessibleName("缩小预览")
        self.btn_zoom_out.setToolTip("缩小预览")
        self.btn_zoom_out.clicked.connect(self._on_zoom_out)
        self.zoom_label = QLabel("100%")
        set_ui_role(self.zoom_label, "muted")
        self.zoom_label.setAccessibleName("当前缩放比例")

        for widget in (self.btn_fit, self.btn_zoom_in, self.btn_zoom_out, self.zoom_label):
    ...
    def _build_extract_tools_panel(self):
        ...
        set_ui_role(self.extract_mode_label, "muted")
        self.extract_mode_label.setAccessibleName("当前采集模式")
        ...
        set_ui_role(help_label, "muted")
        layout.addWidget(help_label)

        layout.addWidget(self._build_picker_toolbar())
        layout.addWidget(self._build_polygon_toolbar())
        layout.addWidget(self._build_roi_toolbar())

        self.roi_apply_label = QLabel("")
        self.roi_apply_label.setWordWrap(True)
        set_ui_role(self.roi_apply_label, "muted")
        self.roi_apply_label.setAccessibleName("ROI 写回状态")
        layout.addWidget(self.roi_apply_label)
        return panel

    def _build_picker_toolbar(self):
        tools = ActionBar()
        tools.setAccessibleName("坐标拾取工具栏")
        ...
        self.btn_picker.setAccessibleName("开始坐标拾取")
        self.btn_picker.setToolTip("在预览图上点击采集像素坐标")
        ...
        self.btn_copy_all.setAccessibleName("复制所有坐标点")
        self.btn_copy_all.setToolTip("复制已采集的全部坐标")
        self.btn_copy_all.clicked.connect(self._on_copy_all_points)

        self.btn_clear_points = QPushButton("清除")
        self.btn_clear_points.setAccessibleName("清除坐标点")
        self.btn_clear_points.setToolTip("清除已采集的坐标点")
        self.btn_clear_points.clicked.connect(self._on_clear_points)

        self.coord_label = QLabel("")
        set_ui_role(self.coord_label, "muted")
        self.coord_label.setAccessibleName("当前坐标")

        self.points_label = QLabel("")
        set_ui_role(self.points_label, "muted")
        self.points_label.setAccessibleName("已采集坐标摘要")

        for widget in (section, self.btn_picker, self.btn_copy_all, self.btn_clear_points,
                       self.coord_label, self.points_label):
            tools.add_widget(widget)
        return tools

    def _build_polygon_toolbar(self):
        poly_bar = ActionBar()
        poly_bar.setAccessibleName("多边形绘制工具栏")
        ...
        self.btn_polygon.setAccessibleName("开始多边形绘制")
        self.btn_polygon.setToolTip("点击预览图添加多边形顶点，双击或点击起点关闭")
        ...
        self.btn_undo_polygon.setAccessibleName("撤销上一个多边形")
        self.btn_undo_polygon.clicked.connect(self._on_undo_polygon)

        self.btn_clear_polygon = QPushButton("清除所有")
        self.btn_clear_polygon.setAccessibleName("清除所有多边形")
        self.btn_clear_polygon.clicked.connect(self._on_clear_polygon)

        self.btn_copy_polygon = QPushButton("复制多边形坐标")
        self.btn_copy_polygon.setAccessibleName("复制多边形坐标")
        self.btn_copy_polygon.setToolTip("复制当前全部多边形坐标")
        self.btn_copy_polygon.clicked.connect(self._on_copy_polygon)

        self.polygon_info = QLabel("")
        set_ui_role(self.polygon_info, "muted")
        self.polygon_info.setAccessibleName("多边形绘制状态")

        for widget in (section, self.btn_polygon, self.btn_undo_polygon,
                       self.btn_clear_polygon, self.btn_copy_polygon, self.polygon_info):
            poly_bar.add_widget(widget)
        return poly_bar

    def _build_roi_toolbar(self):
        roi_bar = ActionBar()
        roi_bar.setAccessibleName("ROI 框选工具栏")
        ...
        self.btn_rect_select.setAccessibleName("开始 ROI 框选")
        self.btn_rect_select.setToolTip("在预览图上拖动框选 ROI")
        ...
        self.btn_undo_rect.setAccessibleName("撤销上一个 ROI")
        self.btn_undo_rect.clicked.connect(self._on_undo_rect)

        self.btn_clear_rect = QPushButton("清除所有")
        self.btn_clear_rect.setAccessibleName("清除所有 ROI")
        self.btn_clear_rect.clicked.connect(self._on_clear_rect)

        self.btn_copy_rois = QPushButton("复制所有ROI坐标")
        self.btn_copy_rois.setAccessibleName("复制所有 ROI 坐标")
        self.btn_copy_rois.setToolTip("复制当前全部 ROI 坐标")
        self.btn_copy_rois.clicked.connect(self._on_copy_rois)

        self.btn_save_rois = QPushButton("保存所有ROI")
        self.btn_save_rois.setAccessibleName("保存所有 ROI")
        self.btn_save_rois.setToolTip("保存当前全部 ROI")
        self.btn_save_rois.setProperty("buttonRole", "primary")
        self.btn_save_rois.setProperty("primary", "true")
        self.btn_save_rois.clicked.connect(self._on_save_rois)

        self.roi_info = QLabel("")
        set_ui_role(self.roi_info, "muted")
        self.roi_info.setAccessibleName("ROI 框选状态")

        for widget in (section, self.btn_rect_select, self.btn_undo_rect,
                       self.btn_clear_rect, self.btn_copy_rois, self.btn_save_rois,
                       self.roi_info):
            roi_bar.add_widget(widget)
        return roi_bar

    def _build_tabs(self):
        self.tabs = QTabWidget()
        self.tabs.setAccessibleName("图像预览标签页")
        self.original_view = ZoomableLabel()
        self.result_view = ZoomableLabel()
        self.original_view.setAccessibleName("原图预览画布")
        self.result_view.setAccessibleName("处理后预览画布")
    ...
    def set_roi_apply_feedback(self, text: str, *, level: str = "info") -> None:
        tone = {"success": "success", "warning": "warning"}.get(level, "info")
        self.roi_apply_label.setStyleSheet(status_badge_stylesheet(tone))
    ...
    def _update_extract_mode_hint(self) -> None:
        if self.btn_picker.isChecked():
            ...
            self.extract_mode_label.setStyleSheet(status_badge_stylesheet("danger"))
        elif self.btn_polygon.isChecked():
            self.extract_mode_label.setText("当前模式：多边形绘制")
            self.extract_mode_label.setStyleSheet(status_badge_stylesheet("success"))
        elif self.btn_rect_select.isChecked():
            self.extract_mode_label.setText("当前模式：ROI 框选")
            self.extract_mode_label.setStyleSheet(status_badge_stylesheet("info"))
        else:
            self.extract_mode_label.setText("当前模式：未启用")
            self.extract_mode_label.setStyleSheet(status_badge_stylesheet("neutral"))
